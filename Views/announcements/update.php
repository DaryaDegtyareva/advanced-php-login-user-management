<div class="my-3 my-md-5">	
	<div class="container">
		<div class="row">
			<div class="col-12"><div class="card">
					<div class="card-header">
						<h3 class="card-title">Edit Announcement: <?php echo $this_obj->name; ?></h3><div class="card-options">
							<a href="<?php echo base_url("announcements"); ?>" class="btn btn-secondary ml-2"><i class="fa fa-chevron-left"></i> &nbsp;&nbsp;Back</a>
						</div>
					</div>
		
		<form action="<?php echo base_url('announcements/update/'.$this_obj->id.'/'.$siteGuard->csrf); ?>" method="POST" enctype='multipart/form-data' >
		<div class="card-body row">
			...
			<div class="col-6">
				<div class="form-group">
                    ...
                    <div class="">
						<input type="text" class="form-control" id="inputName" name="name" placeholder="Name" value="<?php echo $this_obj->name; ?>" required>
                    </div>
				</div>	
				<div class="form-group">
						...
						<select class="form-control select2" name="expire_after" >
							<?php foreach($options as $key => $value) {
								echo "<option value='{$key}'";
									if($this_obj->expire_after == $key) { echo 'selected'; }
								echo ">{$value}</option>";
							} ?>
						</select>
                    </div>
				  ...
				  <div class="form-group">
                    ...
                    <div class="">
						...
						<select class="form-control select2" name="type" >
							<?php foreach($options as $key => $value) {
								echo "<option value='{$key}'";
									if($this_obj->type == $key) { echo 'selected'; }
								echo ">{$value}</option>";
							} ?>
						</select>
                    </div>
				...
				<div class="form-group">
                    ...
                    <div class="">
						<?php 
							...
							$groups_arr = explode(',' , $this_obj->visible_to);
						?>
						<select class="form-control select2" name="visible_to[]" multiple>
							<?php foreach($groups as $group) {
								echo "<option value='-{$group->id}-' ";
									if(in_array('-'.$group->id.'-', $groups_arr)) { echo ' selected'; }
			...
			<div class="form-group">
				...
				<div class="">
					<textarea name="message" class="summernote"><?php 
							$message = html_entity_decode(urldecode($this_obj->message)); 
		...
		<div class="card-footer text-right">
		  <div class="d-flex">
			<button type="submit" name="update_announcement" class="btn btn-primary ml-auto">Update Announcement</button>
		  </div>
		</div>
		<?php 
